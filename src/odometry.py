...
class Odometry:
    ...
    def __get_diff_in_cm(self, tu1: Tuple[int, int], tu2: Tuple[int, int]) -> Tuple[float, float]:
        """
        Returns the difference between two motor positions converted to cm traveled
    ...
    def update_position(self, motor_positions: List[Tuple[int, int]]) -> None:
        """
    ...
    def get_norm(self, v1: Tuple[float, float], v2: Tuple[float, float]) -> float:
        """
    ...
    def __clip_coordinat(self, x: float, y: float, color: Color, planet: Planet) -> Tuple[int, int]:
        ...
        self.logger.debug(f"node color: {color}, Planet check: {planet.check_node_color((floored_x, floored_y), color)}")
    ...
    def set_coordinates(self, position: Tuple[Tuple[int, int], Direction]) -> None:
        """
